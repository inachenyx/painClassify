...
# === Early SMOTE (intentional leakage) ===
# Upsample severe class (assumed label 2) to match the larger of mild(0)/moderate(1)
counts = np.bincount(y)
target = int(max(counts[0], counts[1]))   # match majority of {mild, moderate}

sm = SMOTE(sampling_strategy={2: target}, random_state=42, k_neighbors=5)

# Apply to the entire dataset BEFORE any CV or splitting
X_res, y_res = sm.fit_resample(X, y)

# Groups: extend to match new rows so downstream StratifiedGroupKFold still works.
# We assign each synthetic sample a unique new "pseudo-subject" ID.
n_added = X_res.shape[0] - X.shape[0]
if n_added > 0:
    max_gid = int(np.max(groups))
    groups_res = np.concatenate([groups, np.arange(max_gid + 1, max_gid + 1 + n_added)])
else:
    groups_res = groups

# Overwrite variables used downstream so the rest of your code stays identical
X = X_res
y = y_res
groups = groups_res

print("[SMOTE-early] Class counts after upsampling:", np.bincount(y))
print("[SMOTE-early] Shapes:", X.shape, y.shape, groups.shape)
