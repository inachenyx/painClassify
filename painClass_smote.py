...
# Upsample only the severe class (assumed label==2) to match the larger of mild(0)/moderate(1)
counts = np.bincount(y)
target = max(counts[0], counts[1])  # match mild/moderate size
...
# ---------------------------
def with_smote(model, param_dist):
    if smote is None:
        return model, param_dist
    # Wrap: SMOTE -> classifier
    pipe = ImbPipeline([("smote", smote), ("clf", model)])
    # Prefix params so RandomizedSearchCV can see classifier params inside the pipeline
    if param_dist is None:
        return pipe, None
    param_dist_prefixed = {f"clf__{k}": v for k, v in param_dist.items()}
    return pipe, param_dist_prefixed

best_models = {}
for name, (model, param_dist) in search_spaces.items():
    # NEW: wrap estimator & params so SMOTE happens only on training folds
    model_for_search, param_dist_for_search = with_smote(model, param_dist)
...
# 7. Learning curve
# ---------------------------
def balanced_prefix_cv(cv, X, y, groups, min_classes=2, front_subjects_by_class=None, required_classes=None):
    """
    ...
    for train_idx, test_idx in cv.split(X, y, groups):
        ...
        # 1) Manually seed specific subjects first (optional)
        ...
        # 2) Ensure all required classes are present at the very front
        for c in required_classes:
            ...
            for gid in c_grp_ids:
                if gid not in used_groups:
                    ...
                    front_order.extend(np.where(mask)[0].tolist())
                    used_groups.add(gid)
                    break  # one group for this class is enough

        # 3) Ensure at least `min_classes` classes (if not already satisfied)
...
lc_cv = list(balanced_prefix_cv(cv, X_top20, y, groups, min_classes=2, required_classes=[2]))
